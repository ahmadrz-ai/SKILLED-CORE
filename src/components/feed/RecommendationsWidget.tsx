    ...
    return (
        <AnimatePresence mode="wait">
            {loading ? (
                <motion.div
                    key="loading"
                    className="bg-zinc-900/40 rounded-xl border border-white/5 backdrop-blur-md overflow-hidden mt-4 p-4"
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: "auto" }}
                    exit={{ opacity: 0, height: 0 }}
                    transition={{ duration: 0.3 }}
                >
                    <div className="h-4 bg-white/10 rounded w-1/3 mb-4 overflow-hidden relative">
                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shimmer" style={{ backgroundSize: '1000px 100%' }}></div>
                    </div>
                    <div className="space-y-4">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="flex gap-3">
                                <div className="w-10 h-10 rounded-full bg-white/10 overflow-hidden relative">
                                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent animate-shimmer" style={{ backgroundSize: '1000px 100%' }}></div>
                                </div>
                                <div className="flex-1 space-y-2">
                                    <div className="h-3 bg-white/10 rounded w-2/3 overflow-hidden relative">
                                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shimmer" style={{ backgroundSize: '1000px 100%' }}></div>
                                    </div>
                                    <div className="h-2 bg-white/5 rounded w-1/2 overflow-hidden relative">
                                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shimmer" style={{ backgroundSize: '1000px 100%' }}></div>
                                    </div>
                                </div>
                            </div>
                ...
                <motion.div
                    key="content"
                    className="bg-zinc-900/40 rounded-xl border border-white/5 backdrop-blur-md overflow-hidden mt-4"

...
// Disposable email domains blocklist (core set — extend as needed)
...
/**
 * FIX-001 + FIX-002: Auth-gated candidate search with Ghost Protocol filter.
...
export async function getCandidates(): Promise<Candidate[]> {
    // --- FIX-001: Require authentication ---
    ...
    const users = await prisma.user.findMany({
        where: {
            ...
            // FIX-004: Exclude sandbox/test email domains at DB level
            email: {
                notIn: [],
    ...
    return users
        .filter(user => {
            // FIX-004: Filter test/bot accounts by name pattern
            if (user.name && TEST_NAME_PATTERNS.test(user.name)) return false;

            // FIX-004: Filter placeholder profiles
            ...
            // FIX-018: Profile completeness gate — hide profiles below 40%
        ...
        .map(user => {
            ...
            if (user.ghostMode && recruiterDomain) {
                const candidateEmployerDomain = company.toLowerCase().replace(/\s+/g, '') + '.com';
                // Simple heuristic: match on company name normalized to domain
            ...
            return {
                ...
                matchScore: matchScore,
                verified: false, // Remove mock random verification
